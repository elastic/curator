...
class TestGetRepository(TestCase):
    ...
    def test_get_repository_notfounderror_negative(self):
        client = Mock()
        meta = ApiResponseMeta(status=404, http_version='1.1', headers='', duration=1.0, node='dummy')
        client.snapshot.get_repository.side_effect = NotFoundError('simulated error', meta, 'simulated error')
        self.assertRaises(
            curator.CuratorException,
            curator.get_repository, client, repository=testvars.repo_name
        )
...
class TestCreateRepository(TestCase):
    ...
    def test_raises_exception(self):
        client = Mock()
        client.snapshot.get_repository.return_value = {'not_your_repo':{'foo':'bar'}}
        client.snapshot.create_repository.side_effect = TransportError(500, ('exception', 'reason'))
...
class TestRollableAlias(TestCase):
    def test_return_false_if_no_alias(self):
        ...
        client.indices.get_alias.side_effect = NotFoundError(404, 'simulated error', 'simulated error')
