...
class TestActionShrink_data_node(TestCase):
    ...
    def test_non_data_node(self):
        self.builder()
        self.client.nodes.info.return_value = {'nodes':{self.node_id:{'roles':['ingest']}}}
...
class TestActionShrink_qualify_single_node(TestCase):
    ...
    def test_positive(self):
        ...
        byte_count = 123456
        self.client.nodes.info.return_value = {'nodes':{self.node_id:{'roles':['data']}}}
        self.client.nodes.stats.return_value = {'nodes':{self.node_id:{'fs':{'data':['one'], 'total':{'available_in_bytes':byte_count}}, 'name':self.node_name}}}
...
class TestActionShrink_most_available_node(TestCase):
    ...
    def test_excluded(self):
        self.builder()
        self.client.nodes.info.return_value = {'nodes':{self.node_id:{'roles':['data']}}}
        self.client.nodes.stats.return_value = {'nodes':{self.node_id:{'fs':{'data':['one'], 'total':{'available_in_bytes':self.byte_count}}, 'name':self.node_name}}}
        node_filters = {'exclude_nodes': [self.node_name]}
        shrink = Shrink(self.ilo, node_filters=node_filters)
...
class TestActionShrink_various(TestCase):
    ...
    def test_target_exists(self):
        ...
        self.assertRaises(ActionError, shrink._check_target_exists, testvars.named_index)
    ...
    def test_shard_count(self):
        ...
        self.assertRaises(ActionError, shrink._check_shard_count, testvars.named_index, src_shards)
    def test_shard_factor(self):
        self.builder()
        src_shards = 5
        shrink = Shrink(self.ilo, number_of_shards=3)
        self.assertRaises(ActionError, shrink._check_shard_factor, testvars.named_index, src_shards)
    def test_check_all_shards(self):
        self.builder()
        self.client.cluster.state.return_value = {'routing_table': {'indices': {testvars.named_index: {'shards': {'0': [{'index': testvars.named_index, 'node': 'not_this_node', 'primary': True, 'shard': 0, 'state': 'STARTED'}]}}}}}
        shrink = Shrink(self.ilo, shrink_node=self.node_name)
