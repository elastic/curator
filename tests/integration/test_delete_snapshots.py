...
class TestActionFileDeleteSnapshots(CuratorTestCase):
    def test_deletesnapshot(self):
        ...
        self.write_config(self.args['actionfile'],
            testvars.delete_snap_proto.format(self.args['repository'], 'age', 'creation_date', 'older', ' ', 'seconds', '0', timestamps[0]))
        self.invoke_runner()
        snapshot = get_snapshot(self.client, self.args['repository'], '*')
    ...
    def test_no_repository(self):
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'],
            testvars.delete_snap_proto.format(' ', 'age', 'creation_date', 'older', ' ', 'seconds', '0', ' '))
        self.invoke_runner()
...
class TestCLIDeleteSnapshots(CuratorTestCase):
    def test_deletesnapshot(self):
        ...
        assert 0 == self.run_subprocess(args, logname='TestCLIDeleteSnapshots.test_deletesnapshot')
        snapshot = get_snapshot(self.client, self.args['repository'], '*')
        assert 2 == len(snapshot['snapshots'])
    def test_count_by_age(self):
        ...
        def add_snap(num, name):
            ...
            ilo = IndexList(self.client)
            snap = Snapshot(ilo, repository=self.args['repository'], name=name, wait_interval=0.5)
            snap.do_action()
            snapshot = get_snapshot(self.client, self.args['repository'], '*')
