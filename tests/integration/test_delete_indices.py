...
class TestActionFileDeleteIndices(CuratorTestCase):
    ...
    def test_retention_from_name_days_keep_exclude_false_after_failed_match(self):
        ...
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'],
            testvars.delete_pattern_proto.format('age', 'name', 'older', '\'%Y.%m.%d\'', 'days', -1, r'logstash-\w+-([0-9]+)-[0-9]{4}\.[0-9]{2}\.[0-9]{2}', ' ', ' ', ' '))
        self.invoke_runner()
    ...
    def test_retention_from_name_days_failed_match_with_fallback(self):
        ...
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'],
            testvars.delete_pattern_proto.format('age', 'name', 'older', '\'%Y.%m.%d\'', 'days', 7, '_([0-9]+)_', ' ', ' ', ' '))
        self.invoke_runner()
    ...
    def test_retention_from_name_no_capture_group(self):
        ...
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'],
            testvars.delete_pattern_proto.format('age', 'name', 'older', '\'%Y.%m\'', 'months', -1, '_[0-9]+_', ' ', ' ', ' '))
        self.invoke_runner()
    ...
    def test_retention_from_name_illegal_regex_with_fallback(self):
        ...
        self.create_indices(10)
        self.write_config(
            self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'],
            testvars.delete_pattern_proto.format('age', 'name', 'older', '\'%Y.%m.%d\'', 'days', 3, '_[0-9+_', ' ', ' ', ' '))
        self.invoke_runner()
    ...
    def test_creation_date_newer_than_epoch(self):
        ...
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'],
            testvars.delete_proto.format('age', 'creation_date', 'younger', ' ', 'seconds', 0, ' ', ' ', int(time.time()) - 60))
        self.invoke_runner()
    ...
    def test_delete_in_period(self):
        ...
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'],
            testvars.delete_period_proto.format('period', 'name', '-5', '-1', "'%Y.%m.%d'", 'days', ' ', ' ', ' ', ' ', 'monday'))
        self.invoke_runner()
    ...
    def test_delete_in_period_absolute_date(self):
        ...
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'],
            delete_period_abs.format('period', 'name', '2017.01.01', '2017.01.10', "'%Y.%m.%d'", 'days', "'%Y.%m.%d'", "'%Y.%m.%d'"))
        self.invoke_runner()
    ...
    def test_delete_in_period_intersect(self):
        ...
        self.client.index(index=idx1, id='1', document={'@timestamp': '2017-09-25T01:00:00Z', 'doc' :'Earliest'})
        self.client.index(index=idx1, id='2', document={'@timestamp': '2017-09-29T01:00:00Z', 'doc' :'Latest'})
        self.client.index(index=idx2, id='1', document={'@timestamp': '2017-09-01T01:00:00Z', 'doc' :'Earliest'})
        self.client.index(index=idx2, id='2', document={'@timestamp': '2017-09-29T01:00:00Z', 'doc' :'Latest'})
        ...
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'],
            testvars.delete_period_proto.format('period', 'field_stats', '0', '0', ' ', 'weeks', "'@timestamp'", 'min_value', 'true', 1506716040, 'sunday'))
        self.invoke_runner()
        self.assertEqual(0, self.result.exit_code)
        indices = exclude_ilm_history(get_indices(self.client))
        self.assertEqual(1, len(indices))
    ...
    def test_empty_list(self):
        ...
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'],
            testvars.delete_proto.format('age', 'creation_date', 'older', ' ', 'days', 90, ' ', ' ', int(time.time())))
        self.invoke_runner()
        self.assertEqual(10, len(exclude_ilm_history(get_indices(self.client))))
    ...
    def test_ignore_empty_list(self):
        ...
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'],
            testvars.delete_ignore_proto.format('age', 'creation_date', 'older', ' ', 'days', 90, ' ', ' ', int(time.time())))
        self.invoke_runner()
        self.assertEqual(10, len(exclude_ilm_history(get_indices(self.client))))
        self.assertEqual(0, self.result.exit_code)
    def test_extra_options(self):
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'], testvars.bad_option_proto_test.format('delete_indices'))
        self.invoke_runner()
        self.assertEqual(1, self.result.exit_code)
    def test_945(self):
        self.create_indices(10)
    ...
    def test_allow_ilm_indices_true(self):
        ...
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'],
            testvars.ilm_delete_proto.format('age', 'name', 'older', '\'%Y.%m.%d\'', 'days', 5, ' ', ' ', ' ', 'true'))
        self.invoke_runner()
        self.assertEqual(5, len(exclude_ilm_history(get_indices(self.client))))
    def test_allow_ilm_indices_false(self):
        ...
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'],
            testvars.ilm_delete_proto.format('age', 'name', 'older', '\'%Y.%m.%d\'', 'days', 5, ' ', ' ', ' ', 'false'))
        self.invoke_runner()
        self.assertEqual(10, len(exclude_ilm_history(get_indices(self.client))))
