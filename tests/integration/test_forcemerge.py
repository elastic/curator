...
# pylint: disable=missing-function-docstring, missing-class-docstring, line-too-long, protected-access
import os
...
class TestActionFileforceMerge(CuratorTestCase):
    ...
    def test_extra_option(self):
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'], testvars.bad_option_proto_test.format('forcemerge'))
        self.invoke_runner()
...
class TestCLIforceMerge(CuratorTestCase):
    def test_merge(self):
        ...
        # It apparently can finish testing before the segments have _reported_ as fully merged.
