...
class TestCLICountPattern(CuratorTestCase):
    def test_match_proper_indices(self):
        ...
        self.write_config(
            ...
            DELETE_COUNT_PATTERN.format(r'^(a|b|c)-\d$', 'false', 'name', '\'%Y.%m.%d\'', 'true', 1))
        self.invoke_runner()
    ...
    def test_match_proper_indices_by_age(self):
        ...
        self.write_config(
            self.args['actionfile'],
            DELETE_COUNT_PATTERN.format(r'^(a|b)-\d{4}\.\d{2}\.\d{2}$', 'true', 'name', '\'%Y.%m.%d\'', 'true', 1))
        self.invoke_runner()
        indices = sorted(list(self.client.indices.get(index='*')))
    ...
    def test_count_indices_by_age_same_age(self):
        ...
        alloc = 'include'
        self.write_config(self.args['configfile'], testvars.client_config.format(HOST))
        self.write_config(self.args['actionfile'], testvars.allocation_count_test.format(key, value, alloc, False))
        ...
        for idx in (idx1, idx2):
            assert value == response[idx]['settings']['index']['routing']['allocation'][alloc][key]
        for idx in idxlist:
