...
[[faq_aws_iam]]
== Q: Why doesn't Curator 5 work with AWS Elasticsearch?

[float]
A: Because Curator 5 requires access to the `/_cluster/state/metadata` endpoint.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

There is some confusion because Curator 3 supported AWS ES, but Curator 5 does
...
Curator 5 requires access to the `/_cluster/state/metadata` endpoint in order to
...
(either `open` or `close`), aliases, and more.  Curator 5 switched to doing this
in order to reduce the number of repetitive client calls that were made in the
previous versions.

AWS currently has a 5.1.1 version of Elasticsearch.
Unfortunately, this version does not support the `/_cluster/state/metadata`
