...
NOTE: This option is required by the <<alias,alias action>>.

The value for this setting is the name of the alias to use.
...
Read more about these settings at
...
This setting specifies whether or not the request should return immediately
after snapshot initialization or wait for snapshot completion.
