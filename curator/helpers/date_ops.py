...
def absolute_date_range(
    ...
    """
    This function calculates a date range with an absolute time stamp for both the start time and
    the end time. These dates are converted to epoch time. The parameter ``unit`` is used when the
    same simplified date is used for both ``date_from`` and ``date_to`` to calculate the duration.
    For example, if ``unit`` is ``months``, and ``date_from`` and ``date_to`` are both ``2017.01``,
    then the entire month of January 2017 will be the absolute date range.
...
def date_range(unit, range_from, range_to, epoch=None, week_starts_on='sunday'):
    """
    This function calculates a date range with a distinct epoch time stamp of both the start time
    and the end time in counts of ``unit`` relative to the time at execution, if ``epoch`` is not
    set.

    If ``unit`` is ``weeks``, you can also determine when a week begins using ``week_starts_on``,
    ...
    if not range_to >= range_from:
        raise ConfigurationError('"range_to" must be greater than or equal to "range_from"')
    if not epoch:
        epoch = time.time()
...
def fix_epoch(epoch):
    ...
    try:
        ...
        raise ValueError(f'Bad epoch value. Unable to convert {epoch} to int. {err}') from err
...
def get_point_of_reference(unit, count, epoch=None):
    ...
    :param unit: One of ``seconds``, ``minutes``, ``hours``, ``days``, ``weeks``, ``months``, or
        ``years``.
    :param unit_count: The number of ``units``. ``unit_count`` * ``unit`` will be calculated out to
