...
class ActionError(CuratorException):
    """
    Exception raised when an action (against an index_list or snapshot_list)
    cannot be taken.
    """
...
class ClientException(CuratorException):
    """
    Exception raised when the Elasticsearch client and/or connection is the source
    of the problem.
    """
...
class SearchableSnapshotException(CuratorException):
    """
    Exception raised when Curator finds something out of order with a Searchable
    Snapshot
    """
