...
def delete_callback(ctx, param, value):
    ...
    If the action is ``delete``, this is the :py:class:`click.Parameter` callback
    function if you used the ``--yes`` flag.
    ...
    :type value: str
    """
    # It complains if ``param`` isn't passed as a positional parameter, but this
    # is a one-trick pony callback. We don't need to know that you selected
    # ``--yes`` as the parameter.
    if not value:
        ctx.abort()
...
@click.option(
    ...
    show_default=True,
    type=str,
    help='Azure named client to use.',
)
@click.option(
    '--container',
    ...
    type=str,
    help=(
        'Container name. You must create the Azure container before creating '
        'the repository.'
    ),
)
@click.option(
    ...
    type=str,
    help=(
        'Specifies the path within container to repository data. Defaults to '
...
@click.option(
    ...
    type=click.Choice(['primary_only', 'secondary_only']),
    help='Note that if you set it to secondary_only, it will force readonly to true.',
)
...
@click.option(
    ...
    is_flag=True,
    help='If set, files are encrypted on server side using AES256 algorithm.',
)
@click.option(
    '--buffer_size',
    default='',
    ...
    help=(
        ...
        'request. Must be between 5mb and 5gb.'
    ),
)
@click.option(
    '--canned_acl',
    ...
    help=(
        'When the S3 repository creates buckets and objects, it adds the canned '
        'ACL into the buckets and objects.'
    ),
)
@click.option(
    '--storage_class',
    ...
    help='Sets the S3 storage class for objects stored in the snapshot repository.',
)
@click.option('--verify', is_flag=True, help='Verify repository after creation.')
@click.pass_context
def s3(
...
def fs(
    ...
    create_repo(
        ctx, repo_name=name, repo_type='fs', repo_settings=fs_settings, verify=verify
    )
...
@click.option(
    ...
    default=5,
    show_default=True,
    help='Maximum number of retries for http and https',
)
@click.option(
    '--http_socket_timeout',
    type=str,
    default='50s',
    show_default=True,
    help='Maximum wait time for data transfers over a connection.',
)
@click.option(
    '--compress/--no-compress',
    default=True,
...
@click.option(
    ...
    required=True,
    type=str,
    help='URL location of the root of the shared filesystem repository.',
)
@click.option('--verify', is_flag=True, help='Verify repository after creation.')
@click.pass_context
def url(
...
def source(
    ...
    create_repo(
        ...
        repo_name=name,
        repo_type='source',
        repo_settings=source_settings,
        verify=verify,
    )


# pylint: disable=R0913, R0914, W0622
@click.group(
    'repo_mgr_cli', context_settings=context_settings(), epilog=footer(__version__)
)
